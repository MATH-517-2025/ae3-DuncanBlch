...
From this, it seems like $N$ should depend on the sample size $n$. Indeed, each block fit uses $5$ parameters, so we require $n-5N>0$ and a sufficient number of observations per block (practically $\gtrsim 10$–$15$ in the sparsest block) for stable estimation. As $n$ increases we can afford a larger $N$ without starving blocks; empirically the $C_p$–optimal $\hat N_{\text{opt}}$ increases slowly with $n$. We now want to look into the impact the chosen block size $N$, used in the estimation of $\theta_{22}$ and $\sigma^2$, has on the estimate $\hat{h}_{AMISE}$. We will this time let the sample size take the values $200, 800, 3200$ and $12800$. Moreover, for each $n$, we let $N \in \{1, \ldots, N_{\max} \}$.
